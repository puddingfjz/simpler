...
int DeviceRunner::ensure_binaries_loaded() {
    ...
    // persists. Per-task launches afterwards go through Mode A type 2
    // directly against the preinstall file — no further dispatcher hop.
    ...
    // device allocation avoids AICORE OOM in long test sessions.
